...
import { getStatus, askIfNeeded, openSystemSettings, type MediaPermission } from "./permissions";
...
ipcMain.handle("recording:save", async (_evt, payload: SavePayload): Promise<SaveResult> => {
    const settings = await getSettings();
    saveCounter += 1;
    const base = fillTemplate(settings.filenameTemplate, { counter: saveCounter });
    ...
    if (settings.autoSave || payload.autoSave) {
        ...
        const res = await dialog.showSaveDialog((mainWin ?? undefined) as BrowserWindow, {
...
ipcMain.handle("dialog:pickDirectory", async (_evt, current?: string) => {
    const res = await dialog.showOpenDialog((mainWin ?? undefined) as BrowserWindow, {
...
ipcMain.handle("recents:exists", async (_evt, filePath: string) => existsSync(filePath));

// ---------------------------------------------------------------------------
// IPC — Recording phase (for tray badge + window hide)
// ---------------------------------------------------------------------------
ipcMain.handle("recording:phase", async (_evt, phase: "idle" | "recording" | "paused") => {
...
ipcMain.handle("perm:ask", async (_evt, kind: MediaPermission) => await askIfNeeded(kind));
