...
export function useRecorder(): UseRecorderApi {
    ...
    const setPreview = (s: MediaStream | null) => { previewStreamRef.current = s; setStream(s); };

    const emitTrackDropped = useCallback(() => {
        for (const cb of trackDropListeners.current) cb();
    }, []);

    const arm = useCallback(async ({ source, audio, codec, quality, bitrate, overlays }: ArmArgs) => {
        setError(null);
        setPhase("arming");
        try {
            ...
            if (overlays.cursorHighlight || overlays.keystrokeOverlay || overlays.webcam) {
    ...
    const teardown = () => {
        if (compositorRef.current) {
            compositorRef.current.refs.webcamStream?.getTracks().forEach((t) => t.stop());
    ...
    const onTrackDropped = useCallback((cb: () => void) => {
        ...
        return () => { trackDropListeners.current.delete(cb); };
    }, []);
