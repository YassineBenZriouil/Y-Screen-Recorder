  ...
  const draw = () => {
    ...
    if (flags.keystrokeOverlay) {
      ...
      if (refs.keystrokes.length > 0) {
        drawKeystrokes(
          ctx,
          refs.keystrokes.slice(-MAX_KEYSTROKES),
          now,
          canvas.width,
          canvas.height,
        );
...
function drawCursor(
  ...
  if (localX < 0 || localY < 0 || localX > p.displayW || localY > p.displayH)
    return;
...
function drawWebcam(
  ...
  const aspect =
    cam.videoHeight && cam.videoWidth
      ? cam.videoHeight / cam.videoWidth
      : 9 / 16;
  ...
  switch (corner) {
    case "tl":
      ...
      break;
    case "tr":
      cx = canvasW - camW - margin;
      cy = margin;
      break;
    case "bl":
      cx = margin;
      cy = canvasH - camH - margin;
      break;
    case "br":
      cx = canvasW - camW - margin;
      cy = canvasH - camH - margin;
